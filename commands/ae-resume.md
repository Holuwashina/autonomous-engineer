...
1. Identify the run: filter `TaskList` by the ticket ID, or take the most recent in-progress run. If none, reply `No run to resume. Start one with /ae-start <id> --base <branch>.` and stop.
