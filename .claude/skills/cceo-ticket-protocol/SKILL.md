---
name: cceo-ticket-protocol
description: How CCEO interacts with ticket systems (Jira, ClickUp, GitHub Issues) — fetch, comment, transition, attach evidence. Provider-specific conventions for the Engineering Director and Engineering Manager.
---

# Ticket Protocol

CCEO treats tickets as first-class. Every run touches the ticket at intake and at close-out. The exact commands depend on which ticket MCP is configured.
...
- **Default: do not transition.** Many teams have nuanced workflows (peer review states, QA states, deployment gates) CCEO can't infer.
- **Transition only when** the project's CCEO config explicitly maps "PR opened" → a target status. If unsure, comment a recommendation:

  > "Recommend transitioning this to **In Review**."

- Never close a ticket from CCEO. The user decides when the work is done.
...
- Custom fields (Story Points, Sprint, Components) are read-only from CCEO unless the user requests otherwise.
...
- Closing an issue is **never** automatic — even if a PR auto-closes on merge, that's the user's PR + repo config, not CCEO's action.
