...
- **Orchestrator is now the main session loop, not a subagent.** `/ticket`, `/bug`, `/feature`, and `/resume` load the new `orchestration` skill and drive the run from the main loop. This fixes the v1 flaw where the Engineering Director (a subagent) was expected to spawn subagents — something the runtime does not reliably allow.
...
- `install.sh` and `/setup` install-detection markers updated from the removed `engineering-director.md` to v2 files.
