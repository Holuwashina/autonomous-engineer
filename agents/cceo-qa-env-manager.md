...
<process>
1. **Read `.cceo/resources.yaml`.** If missing, return an error pointing the user at `/setup`. Do not improvise from anywhere else.
2. **Refer to the `cceo-resources` skill** for the schema and selection rules.
...
<rules>
1. **`.cceo/resources.yaml` is the source of truth.** If it's missing, do not improvise — direct the Director to `/setup`.
...
9. You **read only.** Never modify `.cceo/resources.yaml`.
</rules>
