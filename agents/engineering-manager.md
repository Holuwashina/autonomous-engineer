...
6. **Update the ticket** via the ticket MCP (Jira / ClickUp / GitHub Issues):
   ...
   - **Attach QA's validation screenshots to the ticket.** Take the screenshot file paths QA listed (under `.ae/runs/<run-id>/evidence/`) and upload them to this ticket via the provider's attach tool (ClickUp `attach_task_file`, Jira add-attachment, GitHub by embedding the uploaded image in the comment). Attach the key success state per breakpoint (mobile/tablet/desktop) for UI work plus any failure shot; reference them inline in the validation section so a reviewer sees the evidence in the ticket itself. If a file is missing or the provider has no attach capability, say so in the comment and link the local evidence path instead — never claim a screenshot is attached when it isn't.
