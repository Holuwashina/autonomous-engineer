...
How to test (for QA):
- Preconditions / data: <accounts, seed data, env + URL>
- Steps:
  1. <concrete action>
  2. <action>
- Expected result: <what QA should see — the fix's observable behaviour>
- Edge / negative: <e.g. non-manager denied; empty input>
- Screens (UI only): check at mobile / tablet / desktop

Validation we ran:
- <acceptance criterion / repro> — <pass/fail> — <evidence link / attached screenshot>
- ...
Screenshots attached: <list the attached files, e.g. checkout-desktop.png, checkout-mobile.png — or "n/a (no UI surface)">
...
The **How to test** block is mandatory: derive it from QA's validation journey so a human QA can re-verify by hand. Steps must be concrete (exact account, URL, input, expected output), not "test the feature."

### Attach QA's validation screenshots

QA saves its validation screenshots as files under `.ae/runs/<run-id>/evidence/`
(one per meaningful state, per breakpoint for UI). The Engineering Manager
**uploads them to the ticket** alongside the close-out comment, and references them
in the *Validation we ran* section so the evidence lives in the ticket, not only in
the local run log. Attach the key success state per breakpoint (mobile / tablet /
desktop) for UI work plus any failure shot. Skip any capture that shows a real
secret on screen. If the provider can't attach files (or a path is missing), note
it in the comment and link the local evidence path instead — never imply a
screenshot is attached when it isn't.

Per provider: **ClickUp** — `attach_task_file` (task id + file path). **Jira** —
the add-attachment endpoint, then reference by filename in the comment. **GitHub
Issues** — upload the image and embed it in the markdown comment (`![desktop](url)`).
