---
name: cceo-pr-protocol
description: CCEO Pull Request conventions — title format, body template, test plan checklist, draft vs ready, ticket linking. Used by the Engineering Manager when opening a PR.
---

# PR Protocol

CCEO PRs are written to be useful to the human reviewer on GitHub — not as a marketing summary of what CCEO did.
...
---
*Opened by CCEO. The full run trace lives in the Claude Code session.*
```
...
- **Never.** Even if the project has auto-merge enabled, CCEO does not enable it on the PR. The user decides when to merge.
...
- Linking the ticket but not also commenting on the ticket with the PR link (bidirectional linking is mandatory — that lives in `cceo-ticket-protocol`).
