...
If validation or any reviewer returns blocking findings, re-invoke `software-engineer` with the *specific* findings, then re-validate and re-review. Only **blocking** findings loop; non-blocking ones are recorded as PR follow-ups, never iterated on. Reuse the cached intake + repo map — never recompute them. Cap: 2 iterations (T1) / 3 (T2) without convergence → escalate.

**Re-review scope on a loop — re-run the flagging lens *plus* any lens the new fix newly implicates.** Always re-run the lens(es) that raised the blocker, on the new diff. In addition, inspect what the fix *changed* and spawn any other lens whose surface it now touches — even if that lens passed (or never ran) before: a fix that adds a DB query or hot-path work pulls in **perf**; one that touches auth/validation/secrets/persistence pulls in **security** (mandatory on T2); one that shifts a module boundary, public contract, or migration pulls in **arch**; any code change re-runs **code**. The point: a fix can introduce a *new* class of risk, so the lens set can grow across iterations — never assume a previously-green lens stays green after the code moved. Spawn the re-review lenses in one parallel response alongside re-validation (Step 4).

**Final-pass full re-validation (mandatory gate before close-out).** Intermediate loop iterations may re-validate just the changed area for speed (Speed §4), but **the iteration that yields the green verdict must be a full re-validation** — QA re-runs the complete automated suite + type-check *and* re-walks the full acceptance journey (all breakpoints for UI), not only the changed slice. This guards against a fix that quietly regresses something outside the touched area. Only after a *full* `pass` does the run go to the Engineering Manager. If the run converges on the first try (no loop), that single validation is already full, so this adds nothing; it only bites when a targeted re-check was used mid-loop.
...
4. **Targeted re-validation on *intermediate* loops.** On iteration 2+ that is *not* the final pass, re-validate only the changed area, not the whole journey/suite. Reuse the cached intake + repo map — never recompute. **But the final iteration before close-out is always a full re-validation** (full suite + type-check + complete acceptance journey) — see Step 5; speed yields to the regression guard for the last pass only.
5. **Don't reload context — pass artifacts downstream, don't re-derive.** Give QA and the reviewers the **intake repo map** and the engineer's **codebase-findings** note so they don't re-scan structure. QA reuses its environment/account selection across `reproduce`→`validate` (don't re-pick). Point QA and the engineer at the **persistent journey map** (`.ae/journeys/`, `journey-map` skill) — the system's standing record of how each part is navigated with known inputs/outputs, so QA starts from known navigation instead of re-discovering it each run; both update it at hand-off so it compounds across tickets. Load a skill only when its branch needs it.
